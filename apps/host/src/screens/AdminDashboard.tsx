...
export function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] font-body flex flex-col">
      <header className="bg-[var(--color-surface-elevated)]/80 backdrop-blur-md shadow-sm border-b border-[var(--color-border)]/50 py-4 px-4 sm:px-8 flex justify-between items-center sticky top-0 z-50">
        ...
        <Link 
          to="/" 
          className="text-[var(--color-text-secondary)] font-semibold text-sm hover:text-[var(--color-text-primary)] transition-colors"
        >
          &larr; Back to Host
        </Link>
      </header>

      <main className="flex-1 p-8 flex flex-col items-center animate-[screenEnter_300ms_var(--ease-out-expo)] max-w-7xl mx-auto w-full">
        ...
        {creating && (
          <div className="w-full bg-[var(--color-surface-elevated)] p-6 rounded-2xl shadow-[0_1px_2px_rgba(43,38,32,0.04),0_8px_24px_rgba(43,38,32,0.06)] border border-[var(--color-border)] mb-8 animate-[screenEnter_200ms_var(--ease-out-expo)]">
        ...
        {loading && quizzes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
            {quizzes.map((quiz) => (
              <div 
                key={quiz.id} 
                className="bg-[var(--color-surface-elevated)] p-6 rounded-2xl shadow-[0_1px_2px_rgba(43,38,32,0.04),0_8px_24px_rgba(43,38,32,0.06)] border border-[var(--color-border)] flex flex-col transition-all duration-300 relative group"
              >
                <div className="flex-1">
                  <h3 className="text-xl font-heading font-bold mb-2 text-[var(--color-text-primary)] line-clamp-2">{quiz.title}</h3>
                  <div className="inline-block bg-[var(--color-accent)]/10 text-[var(--color-accent)] font-semibold py-1 px-3 rounded-full text-sm mb-4">
                ...
                {deletingId === quiz.id ? (
                  <div className="flex flex-col gap-2 animate-[screenEnter_200ms_var(--ease-out-expo)]">
                  ...
                  <div className="flex gap-3">
                    <button 
                      ...
                      className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-primary)] py-3 rounded-xl font-bold hover:bg-[var(--color-border)] hover:border-[var(--color-text-secondary)] transition-all ease-[var(--ease-smooth)] active:scale-95"
                    >
                      Edit Questions
                    </button>
                    <button 
