...
export function TerminatedScreen() {
  const terminatedData = usePlayStore(state => state.terminatedData);
  const participantId = usePlayStore(state => state.participantId);

  const leaderboard = terminatedData?.finalLeaderboard || [];
  const me = leaderboard.find(e => e.id === participantId);
  const myRank = me?.rank || null;
  const targetScore = me?.score || 0;
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-[90vh] px-4 py-8 text-center bg-[var(--color-surface)]">
      ...
      <div className="mb-4 bg-amber-100 border border-amber-300 text-amber-900 px-6 py-3 rounded-2xl w-full max-w-sm shadow-sm">
        <span className="text-2xl block mb-1">🛑</span>
        <p className="font-bold">This quiz was ended early by the host.</p>
      </div>
