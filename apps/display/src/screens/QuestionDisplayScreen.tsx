...
export function QuestionDisplayScreen() {
  ...
  const isPaused = useDisplayStore(state => state.isPaused);
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-[var(--color-bg)] py-10 px-8 sm:px-12 md:px-16 overflow-hidden relative">
      {isPaused && (
        <div className="absolute inset-0 z-50 bg-[var(--color-bg)]/50 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[var(--color-surface-elevated)] p-12 rounded-3xl shadow-[0_0_60px_rgba(0,0,0,0.2)] border-4 border-amber-300 max-w-2xl w-full text-center animate-[popIn_300ms_var(--ease-spring)]">
            <div className="text-8xl mb-6">⏸</div>
            <h3 className="text-6xl font-heading font-black text-[var(--color-text-primary)] mb-4 uppercase tracking-wider">Paused</h3>
            <p className="text-3xl text-[var(--color-text-secondary)] font-bold">Waiting for host to resume...</p>
          </div>
        </div>
      )}
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col h-full">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className={`w-20 h-20 lg:w-24 lg:h-24 rounded-full flex items-center justify-center text-4xl lg:text-5xl font-heading font-black shadow-inner
        ...
        <div className="bg-[var(--color-surface-elevated)] p-10 lg:p-12 rounded-2xl shadow-xl border border-[var(--color-border)] mb-8 text-center flex-shrink-0 relative overflow-hidden">
          ...
          <h2 className="text-3xl lg:text-4xl font-heading font-black text-[var(--color-text-primary)] leading-tight">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-4 lg:gap-6 flex-1 min-h-0 pb-4">
          {currentQuestion.options.map((opt, i) => {
            ...
            return (
              ...
              >
                <div className="bg-[var(--color-surface-elevated)]/20 p-4 lg:p-5 rounded-2xl flex-shrink-0 shadow-inner">
                  <OptionIcon type={style.icon} className="w-10 h-10 lg:w-12 lg:h-12 text-white drop-shadow-md" />
                </div>
                <span className="text-2xl lg:text-3xl font-bold text-left drop-shadow-md leading-tight line-clamp-3">
                  {opt.text}
                </span>
              </div>
            );
